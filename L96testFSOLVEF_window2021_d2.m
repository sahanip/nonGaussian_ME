function [diff, ey_est, xcovs, eyall] = L96testFSOLVEF_window2021_d2(xi, exhid, F, K, tstep, yobs, obsind,hidind, windl, obsfreq, bw)

%based on L96testFSOLVEF_window2021 but now considers 2 dimensional
%covariate for case study 2. 
%ASSUMPTION: ADOPT PRODUCT KERNEL FOR SIMPLICITY. 
...
%bw = 1 x c vector of bandwidths of the gaussian kernel i.e. standard deviation, is a vector the same size as the no. of dimensions of the covariate. - FIXED
%BANDWIDTH, MAY NEED TO DO ADAPTIVE LATER! 
...
xcovstemp = [xcovs(end,:); [xcovs(1:end-1,:)]];  %other spatial covariate.
Xd = [xcovs(:), xcovstemp(:)];  %m x 2 covariate matrix. 
Yd = eyall(:);
diff = 0;
%now estimate conditional variance using kernel:
for i = 1:length(Xd)
    % calc cond mean - using product kernel. 
    Kb = normpdf(Xd(:,1), Xd(i,1), bw(1)).*normpdf(Xd(:,2), Xd(i,2), bw(2));
    normK = Kb/sum(Kb);
