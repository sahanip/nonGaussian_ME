...
%benchmark - method B2:
load assimilateresults_CS2_B2.mat
%xfitb = xfit; normfplotb = normfplot; 
ensstatesb = ensstates;
clearvars -except ensstatesm ensstatesb

%benchmark - method B1
load assimilateresults_CS2_B1_alpha=0.8.mat

load figure8colormap.mat 
...
ylims11 = [-0.4 0.4];   %obs, proposed method 
ylims12 = [-0.4 0.4];   %obs, method B2
ylims13 = [-1.2 1.2];     %obs,  method B1

ylims21 = [-1.6 1.6];   %hid,  proposed method 
ylims22 = [-1.6 1.6];   %hid,  method B2
ylims23 = [-2.5 3.9];     %hid, method B1
...
figure 
subplot(nrows,ncols,1)  %proposed approach, obs var
...
subplot(nrows,ncols,2)  %proposed approach hid var
...
subplot(nrows,ncols,4)  %benchmark, hid var
...
ylim(ylims23)
xlim(xlims)
set(gca, 'YTick', round(linspace(ylims23(1), ylims23(2), nolabs)*100)/100);
hold on 
plot([1:plottime], zeros(1, plottime), 'k')
grid on 
set(gca, 'FontSize', fs,'TickLength',[0.025 0.025], 'LineWidth', 1.8)
