%lead time forecasts Assimilation CS1 METHOD b1
...
load trainresults_CS1_B1_lambda=1.45.mat  %B AND PVEC DATA
...
for ml = 1:noICs
    ...
    clearvars -except ml simlength maxleadtime bmvec driftall Pmmat xt1all lambda1 maxinf ro_loc alpha plotfigs tstep K Fcons hx hy obsfreq *_full LSall CRPSall RMSEall ENSVARall ENSMEANERRall noICs obserrvar BINPOSall nobins *_anal
    ...
    for i = 1:simlength 
        ...
        for lt = 1:maxleadtime2 
            clear xfit ffit mtemp etemp predic2 artemp ftmp 
            if lt == 1
                ...
                while ~isempty(unph)
                    enstemp = predic - alpha*mvnrnd(repmat(bmvec, 1, n)', repmat(Pmmat, [1, 1, n]))';
                    ensold = predic2;
                    ensold(unph) = enstemp(unph);
                    predic2 = ensold;
                    clear enstemp ensold 
...
save(['forecastresults_CS1_B1_alpha=', num2str(alpha), '.mat'], 'LSall', 'CRPSall', 'ENSMEANERRall', 'ENSVARall', 'maxleadtime', 'nobins', '*_anal')
