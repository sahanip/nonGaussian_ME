function [diff,xstart] = L96testFSOLVEF_window10(xi, exguess, F, K, tstep, yobs, obsind, windl, obsfreq, RI, QI)
...
%OPTION 3: WEAK CONSTRAIN 4DVAR TYPE SETUP:

for m = 1:windl
    J(m) = 0.5*(exguess(:,m)'*QI*exguess(:,m)) + 0.5*(((xstart(obsind,m+1) - yobs(:,m+1))')*RI*(xstart(obsind,m+1) - yobs(:,m+1)));    
end
diff = sum(J);
