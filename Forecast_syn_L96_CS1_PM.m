%lead time forecasts Assimilation CS1 Proposed method 
...
load traininputdata_syn_L96_CS1.mat  %evaluation period data!
...
nobins =10;  %for rank histogram 

for ml = 1:noICs
    
    clearvars -except simlength ml maxleadtime *_ar bw bwm covindex datatrain plotfigs tstep K Fcons hx hy obsfreq *_full LSall CRPSall RMSEall ENSVARall noICs obserrvar ENSMEANERRall BINPOSall nobins *_anal 
    ...
    for i = 1:simlength 
        ...
        for lt = 1:maxleadtime2 
            [ml, i, lt]
            clear xfit ffit mtemp predic2 artemp arcov prediccov 
            if lt == 1
                %already have one step ahead prediction:
                predic = ensstates(:,:,i+1);
                etemp = (statesA(:,:,i+1) - tempensstates(:,:,i+1));
                ...
                for p = 1:novars                    
                    ...
                    covvaltemp = [prediccov(p,:)', arcov(p,:)'];
                    ...
                    %add error and check if physical:
                    etemp(p,:) = temperrest;
                    predic2(p,:) = etemp(p,:) + predic(p,:); %range of q based on model error for q
                    unph = find(or(predic2(p,:) < stbound(p,:,1), predic2(p,:) > stbound(p,:,2)));
                    while ~isempty(unph)
                        %resample:
                        etemp(p,unph) = sampleCKDE_multcov_singres(datatrain(:,[fixedres,covindex(fixedres,1:size(covindex,2))]), repmat(bw(fixedres,:), size(datatrain,1),1), 1, covindex, covvaltemp(unph,:));                             
                        predic2(p,unph) = etemp(p,unph) + predic(p,unph); %range of q based on model error for q
                        unph = find(or(predic2(p,:) < stbound(p,:,1), predic2(p,:) > stbound(p,:,2)));
                    end 
                    
                    %checks to make sure covering everything 
                    if ~isempty(find(isnan(etemp)))
...
save('forecastresults_CS1_PM.mat', 'LSall', 'CRPSall', 'ENSMEANERRall', 'ENSVARall', 'maxleadtime', 'nobins', '*_anal', 'x_true')
