%execute model error estimation with method B2 for case study 1 and
...
%first collate into one long timeseries:
errxtemp = [];
for m = 1:noICs
    errxtemp = [errxtemp, errx_true(:,find(~isnan(errx_true(1,:,m))),m)];
end
...
save('alldata_train_obserr_CS1_B2.mat')
save('trainresults_obserr_CS1_B2_forR.mat', 'alldata')
save('trainresults_obserr_CS1_B2_forR_true.mat', 'alldata_true')
